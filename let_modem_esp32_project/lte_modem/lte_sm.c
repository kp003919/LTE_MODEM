...
//maximum number of retries before escalating to ERROR state
...
{
    (void)response; /* Forward path does not interpret modem responses */
    ...
    case LTE_SM_CHECK_SIM:
        /* SIM ready → set URC and then begin network attach */
    ...
    case LTE_SM_ATTACH:
        ...
        printf("Handling network attach...\n");
        break;
    ...
    case LTE_SM_RUN:
        // In RUN state, the modem is fully initialized and ready for
        //  application data. 
        break;
...
{  
    // check previous state and map to recovery 
    switch (s_prev_state) {
    ...
    // if TCP failed, retry TCP open
    case LTE_SM_OPEN_SOCKET:
        set_state(LTE_SM_REOPEN_SOCKET);
        break;
    
    // if MQTT connect failed, retry MQTT connect
    case LTE_SM_MQTT_CONNECT:
        set_state(LTE_SM_MQTT_RECONNECT);
        break;
    
    // if MQTT subscribe failed, retry MQTT subscribe
    case LTE_SM_MQTT_SUBSCRIBE:
    ...
    default:
        ...
        // Log the unexpected error mapping
...
/* --------------------------------------------------------------------------
 * Handle +CEREG URC (Network Registration Status)
 *
 * The modem may send +CEREG URCs asynchronously to indicate network status.
 * This function interprets the URC and updates the state machine accordingly.
 *
 * Important:
 *   - This is an asynchronous event; it does not complete any command.
 *   - It may trigger recovery if the network is lost.
 * -------------------------------------------------------------------------- */
...
static void lte_handle_cereg_urc(const char *line)
{
    ...
    if (sscanf(line, "+CEREG: %*d,%d", &stat) == 1) {
        // Log the received CEREG status for debugging
        // 1 --> Registered, home network
        // 5 --> Registered, roaming    
        // 0 --> Not registered, not searching  (lost network)
        ...
        default:
            ...
            // safe default: treat as error to trigger recovery
            s_current_cmd = LTE_CMD_NONE;
            set_state(LTE_SM_ERROR);
            break;
        }
...
void lte_sm_handle_event(const LteEvent_t *ev)
{
    ...
    /* ----------------------------------------------------------------------
     * NO EVENT → RUN FORWARD PATH
     * ---------------------------------------------------------------------- */
    if (!ev) {
        lte_sm_forward_path();   // <— send AT, attach, PDP, TCP, MQTT
        return;
    }
    ...
    if (ev->type == LTE_EVENT_RX_LINE) {
        ...
        /* --------------------------------------------------------------
     * 1) Handle URCs FIRST (they are asynchronous)
     * -------------------------------------------------------------- */
