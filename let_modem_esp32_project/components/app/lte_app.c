...
// This code is part of an LTE modem application that manages the state of the 
// modem, handles events, and runs the main application logic. It includes functions 
// for sending TCP and MQTT messages, reading sensor data, and triggering alarms 
// based on predefined thresholds. The application loop runs only when the modem is 
// in the RUN state, ensuring that it is fully initialized and connected to the 
// network before executing any logic.    

// The code also includes a test mode that allows for running a test harness to verify
// the functionality of the LTE modem without executing the main application logic. 
...
{   
    ...
    {  // modem is not ready yet, skip application logic
        printf("mode is not ready yet \n");
        return;
    }          
        
    // Get the current time in milliseconds for timing operations
    uint32_t now = lte_hw_ms();
    // design a json message to hold information 
    // char json_message[256];
    //json_message = "Hello from esp32 ";
    // proper json document will be used for a proper data 
    ...
    if (now - s_last_mqtt > 15000) {
        lte_mqtt_publish("device/telemetry", "Hello from Muhsin");
        s_last_mqtt = now;
    }
    ...
    // Check if any alarm conditions are currently active   
    int alarm_now = (level > MAX_LEVEL) || leak || (pressure < MIN_PRESSURE);
    
    // If an alarm condition has just been triggered (i.e., it was not active in the previous iteration),
    // send an alarm message via TCP and MQTT. This ensures that alarms are only sent when      
    // a new alarm condition arises, preventing repeated messages for the same condition.
 ...
 * This function initializes the LTE modem and enters the main loop, where it
 * continuously processes modem events and runs the application logic.
 *
 * app_main:
 * - Initializes the LTE modem
 * - Enters an infinite loop to handle events and run application logic
 * 
 * - Modes: 
 *   - LTE_TEST_MODE: Runs the LTE modem test harness and halts further execution.  
 *   - Normal Mode: Continuously processes modem events and runs the application logic.
 */
...
void app_main(void)
{
    ...
    for (;;)
    {
        ...
        /* 1. Drain all pending modem events */
